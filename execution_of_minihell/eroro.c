/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   eroro.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: momihamm <momihamm@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/08 21:14:45 by momihamm          #+#    #+#             */
/*   Updated: 2023/11/09 20:35:07 by momihamm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#####################################################
bash-3.2$ export mr++====41258972_()$
bash: syntax error near unexpected token `('
#####################################################
 printf ("%p\n", make_expo("a+="));
    printf ("%p\n", make_expo("b+="));
    printf ("%p\n", make_expo("c+="));
#####################################################
bash-3.2$ export 2abi3a=hello
bash: export: `2abi3a=hello': not a valid identifier
bash-3.2$ export -2abi3a=hello
bash: export: -2: invalid option
export: usage: export [-nf] [name[=value] ...] or export -p
bash-3.2$ export _2abi3a=hello
#####################################################

variable name [_ or alpha] [_ or alpha or number]
the first character should be only _ or alphabet the other characters should be onlt _ or alphabet or numbers
 printf ("%p\n", make_expo("*tabi3a=hello"));

  char st[1000] = "zoq+=" 
  char st[1000] = "@@@@@@@" 